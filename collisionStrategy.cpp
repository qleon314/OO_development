...
bool MidPointCollisionStrategy::execute(
  ...
  int COLLISION_DISTANCE = 
    std::min(obj1.getFrame()->getWidth(), obj2.getFrame()->getWidth());
...
isVisible(unsigned __int32 pixel, SDL_Surface *surface) const {
...
bool PerPixelCollisionStrategy::execute(
  ...
  unsigned __int32 *pixels1 = static_cast<unsigned __int32 *>(surface1->pixels);
  unsigned __int32 *pixels2 = static_cast<unsigned __int32 *>(surface2->pixels);
  unsigned pixel1;
  unsigned pixel2;
