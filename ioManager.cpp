    ...
    inputString("")  
{
  ...
  gameOverFont = TTF_OpenFont(
         gdata.getXmlStr("gameOverFont/file").c_str(), 
         gdata.getXmlInt("gameOverFont/size")
         );
  if ( !font ) {
...
SDL_Surface* IOManager::loadAndSet(const string& filename, bool setcolorkey) const {
  ...
  if ( setcolorkey ) {
    unsigned __int32 colorkey = SDL_MapRGB(tmp->format, 255, 0, 255);
...
void IOManager::printgameoverCenteredAt( const string& msg, unsigned __int32 y) const {
   SDL_Surface *stext = TTF_RenderText_Blended(gameOverFont, msg.c_str(), color);
   if (stext) {
     unsigned __int32 x = ( viewWidth - stext->w ) / 2;
     SDL_Rect dest = {x,y,0,0};
     SDL_BlitSurface( stext, NULL, screen, &dest );
     SDL_FreeSurface(stext);
   }
...
void IOManager::printStringAfterMessage( const string& msg,
       unsigned __int32 x, unsigned __int32 y ) const {
