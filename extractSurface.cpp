...
unsigned __int32 ExtractSurface::getpixel(SDL_Surface *surface, int x, int y) const {
  ...
  switch(bpp) {
    ...
    case 4: return *(unsigned __int32 *)p;
    default: return 0;       // shouldn't happen, but avoids warnings
  }
}

void ExtractSurface::putpixel(SDL_Surface *surface, 
     int x, int y, unsigned __int32 pixel) const {
  int bpp = surface->format->BytesPerPixel;
