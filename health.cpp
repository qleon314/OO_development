...
Health::Health(const std::string & name) :
  ...
  start(Vector2f(_gd.getXmlInt(name+"/x"), 
      _gd.getXmlInt(name+"/y"))),
  totalLength(_gd.getXmlInt(name+"/length")),
  currentLength(totalLength),
  thick(_gd.getXmlFloat(name+"/thick")),
  increments(_gd.getXmlFloat(name+"/increment")),
  interval(_gd.getXmlFloat(name+"/interval")),
  deltaTime(0),
...
void Health::update(unsigned __int32 ticks) {
  deltaTime += ticks;
